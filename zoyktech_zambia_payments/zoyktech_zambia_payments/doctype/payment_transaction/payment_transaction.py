...
class PaymentTransaction(Document):
	...
	def update_sales_invoice_payment(self, invoice_name):
		...
		try:
			...
			frappe.msgprint(
				_("Payment Entry {0} created against Sales Invoice {1}").format(
	...
	def update_purchase_invoice_payment(self, invoice_name):
		...
		try:
			...
			frappe.msgprint(
				_("Payment Entry {0} created against Purchase Invoice {1}").format(
					payment_entry.name, invoice_name
				)
			)
...
def create_payment_transaction(payment_data):
	...
	try:
		...
		if frappe.db.exists("Payment Transaction", payment_data.get("reference")):
			return {"success": False, "message": _("Payment transaction already exists")}
		...
		return {"success": False, "error": str(e), "message": _("Failed to create payment transaction")}
...
def update_payment_status(reference_id, status, transaction_id=None, failure_reason=None):
	...
	try:
		...
		return {"success": True, "message": _("Payment status updated successfully")}
