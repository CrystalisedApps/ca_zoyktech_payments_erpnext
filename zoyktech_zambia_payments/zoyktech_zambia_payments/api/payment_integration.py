...
class PaymentIntegration:
	...
	def create_payment_request(
		self, doctype: str, docname: str, payment_method: str = None, **kwargs
	) -> Dict:
		"""
		...
		try:
			...
			return {"success": False, "error": str(e), "message": _("Failed to create payment request")}
	...
	def create_payment_link(
		self, doctype: str, docname: str, reference_id: str, doc, payment_method: str = None
	) -> str:
	...
	def prepare_payment_data(
		self, doctype: str, doc, reference_id: str, payment_method: str = None, kwargs: dict = None
	) -> Dict:
	...
	def create_payment_transaction(
		self, reference_id: str, payment_data: Dict, gateway_response: Dict
	) -> str:
		"""Create payment transaction record"""
	...
	def process_successful_payment(self, reference_id: str, gateway_data: Dict) -> Dict:
		...
		try:
			...
			return {"success": False, "error": str(e), "message": _("Failed to process payment")}
	...
	def update_payment_link_status(self, reference_id: str, status: str, payment_txn_name: str = None):
		...
		payment_links = frappe.get_all(
			"Payment Link", filters={"reference_id": reference_id}, fields=["name"]
		)
	...
	def update_linked_erpnext_documents(self, reference_id: str, payment_txn):
		...
		for link in payment_links:
			try:
				...
				frappe.db.rollback()
				frappe.log_error(
					f"Error updating {link.linked_doctype} {link.linked_docname}: {str(e)}",
	...
	def update_sales_invoice_payment(self, invoice_name: str, payment_txn):
		...
		try:
			...
			frappe.msgprint(
				_("Payment Entry {0} created for Sales Invoice {1}").format(payment_entry.name, invoice_name)
			)

		except Exception as e:
			frappe.log_error(
				f"Error creating payment entry for Sales Invoice: {str(e)}", "Payment Entry Error"
			)
			raise
	...
	def update_purchase_invoice_payment(self, invoice_name: str, payment_txn):
		...
		try:
			...
			frappe.msgprint(
				_("Payment Entry {0} created for Purchase Invoice {1}").format(
			...
			frappe.log_error(
				f"Error creating payment entry for Purchase Invoice: {str(e)}", "Payment Entry Error"
			)
			raise
	...
	def update_sales_order_payment(self, order_name: str, payment_txn):
		...
		try:
			...
			frappe.msgprint(
				_("Advance Payment Entry {0} created for Sales Order {1}").format(
			...
			frappe.log_error(
				f"Error creating advance payment for Sales Order: {str(e)}", "Advance Payment Error"
			)
			raise
	...
	def cancel_payment_request(self, reference_id: str) -> Dict:
		...
		try:
			...
			return {"success": True, "message": _("Payment request cancelled successfully")}
		except Exception as e:
			return {"success": False, "error": str(e), "message": _("Failed to cancel payment request")}
	...
	try:
		...
		if payment_link:
			...
			result = handler.process_successful_subscription_payment(
				subscription_name, payment_link, payment_txn.name
			)

			if result.get("success"):
				frappe.msgprint(
					_("Subscription payment processed: Invoice {0}, Payment {1}").format(
						result.get("sales_invoice"), result.get("payment_entry")
					)
				)
			else:
				frappe.log_error(
					f"Failed to process subscription payment: {result.get('message')}",
