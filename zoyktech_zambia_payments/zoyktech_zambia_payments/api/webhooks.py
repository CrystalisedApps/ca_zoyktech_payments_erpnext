...
def handle_successful_payment(payment_data: dict):
	...
	try:
		...
		if result.get("success"):
			...
			return {"status": "success", "message": "Payment processed successfully", "reference": reference}
		else:
...
def update_payment_link_status(reference: str, status: str):
	...
	payment_links = frappe.get_all("Payment Link", filters={"reference_id": reference}, fields=["name"])
...
def get_webhook_logs(days: int = 7):
	...
	logs = frappe.get_all(
		"Error Log",
		filters={"creation": [">=", since_date], "method": ["like", "%webhook%"]},
